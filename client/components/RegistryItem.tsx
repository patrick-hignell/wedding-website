...
export default function RegistryItem({
  ...
  return (
    <div
      className="alternating m-4 flex w-[90%] flex-col gap-4 rounded-lg border border-black bg-opacity-15 p-4 font-['Bellota'] text-xl sm:w-[500px] lg:w-[45%] xl:w-[50%]"
      style={{
    ...
    >
      <div className="flex flex-col gap-2 text-justify xl:flex-row">
        <div className="w-fill flex justify-center">
          ...
          >
            <img
              className=" w-[300px] border border-black md:w-[400px]"
              alt={registryItem.image}
              src={`/images/${registryItem.image}.png`}
            />
          </a>
        </div>
        <div className="flex flex-1 flex-col items-center justify-evenly text-center text-base md:text-xl">
          <p className="text-2xl font-bold md:text-3xl">{registryItem.name}</p>
      ...
      {myRegistryEntries.map((entry) => (
        <div
          key={entry.id}
          className="flex items-center gap-2 text-[0.8rem] md:text-xl"
        >
      ...
      {selected && myRegistryEntries.length == 0 && (
        <div className="flex flex-col items-center text-center text-base md:text-xl xl:flex-row xl:gap-2">
